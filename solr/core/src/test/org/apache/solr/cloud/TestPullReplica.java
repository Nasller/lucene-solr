...
//    .registerCore(collectionName); //TODO: Is this needed?
  ...
  public void testKillPullReplica() throws Exception {
    ...
    cluster.getSolrClient().getZkStateReader().waitForActiveCollection(collectionName, 5, TimeUnit.SECONDS, false, 1, 1, true, true);
    ...
    cluster.getSolrClient().getZkStateReader().waitForLiveNodes(5, TimeUnit.SECONDS, (newLiveNodes) -> newLiveNodes.size() == 2);
    cluster.getSolrClient().getZkStateReader().waitForActiveCollection(collectionName, 5, TimeUnit.SECONDS, false, 1, 2, true, true);
    waitForNumDocsInAllActiveReplicas(2);
  }
